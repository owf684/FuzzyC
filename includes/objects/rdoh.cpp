...
void RDOH::generate_object(std::string object_name, ImVec2 position) {

	if (object_name == "bird") { 
		bird anyObj = std::any_cast<bird>(object_library["bird"]); 
		 graphics_engine.render_buffer.push_back(std::move(anyObj.generate_object())); 
		 graphics_engine.render_buffer[graphics_engine.render_buffer.size()-1]->set_position(position.x,position.y);
